...
![Sơ đồ kiến trúc tổng thể FinVantage](../../../images/finvantage-architecture.png)

*Dịch vụ AWS sử dụng*  
- *Amazon API Gateway*: Cổng kết nối REST API an toàn tiếp nhận yêu cầu từ client.  
- *AWS Lambda*: Xử lý logic nghiệp vụ Backend Serverless (Node.js 24.x).  
- *Amazon S3*: Lưu trữ tệp tin hóa đơn/chứng từ bảo mật.  
- *Amazon Textract*: Trích xuất dữ liệu tự động từ hóa đơn (OCR `AnalyzeExpense` API).  
- *Amazon Bedrock*: Tích hợp AI (Claude 3.5 Sonnet) phân loại chi tiêu và tư vấn tài chính.  
- *Amazon RDS (PostgreSQL) & RDS Proxy*: Lưu trữ dữ liệu cấu trúc và tối ưu connection pool.  
- *Amazon ElastiCache (Valkey/Redis)*: Bộ nhớ đệm tốc độ cao cho kết quả OCR và session.  
- *AWS Amplify Hosting & Amazon Cognito*: Hosting ứng dụng React và xác thực người dùng.  
- *Amazon SNS & CloudWatch*: Giám sát hiệu năng hệ thống, thu thập log và gửi cảnh báo.  
