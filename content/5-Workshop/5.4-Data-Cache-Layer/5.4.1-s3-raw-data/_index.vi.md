...
1.  `finvantage-s3-permissions.png` - Chặn truy cập công cộng.
2.  `finvantage-s3-cors.png` - Cấu hình CORS Rules.
3.  `finvantage-s3-trigger.png` - Cấu hình Event Notification kích hoạt Lambda.
4.  `finvantage-s3-objects.png` - File hóa đơn thực tế trong thư mục uploads/.